...
export default function Students() {
    ...
    const [form, setForm] = useState({ name: "", email: "", department: "Computer Science", status: "Active" });
    ...
    const openEdit = (student) => {
        ...
        setForm({ name: student.name, email: student.email, department: student.department, status: student.status });
        setShowModal(true);
        setOpenMenu(null);
    };
    ...
    const handleSave = () => {
        ...
        if (editStudent) {
            ...
            const newStudentId = `STU-2024-00${students.length + 1}`;
            setStudents([...students, { id: newId, studentId: newStudentId, createdAt: Date.now(), ...form }]);
        }
        setShowModal(false);
    };
        ...
        <div style={{ padding: "0 32px 32px", background: "#f5f7f5", minHeight: "100vh", fontFamily: "sans-serif" }}>
            ...
            <div style={{ background: "white", borderRadius: "12px", border: "1px solid #eee" }}>
                ...
                {filtered.map(student => (
                    <div key={student.id} style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr 1.2fr 0.8fr 0.5fr", padding: "16px 20px", borderBottom: "1px solid #f5f5f5", alignItems: "center", position: "relative" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                        ...
                        <div style={{ position: "relative" }}>
                            <button onClick={() => setOpenMenu(openMenu === student.id ? null : student.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: "4px" }}>
                            ...
                            {openMenu === student.id && (
                                <div style={{ position: "absolute", right: 0, top: "100%", background: "white", border: "1px solid #eee", borderRadius: "8px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)", zIndex: 100, minWidth: "140px" }}>
                                    ...
                                    <div onClick={() => handleDelete(student.id)} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "10px 14px", cursor: "pointer", fontSize: "13px", color: "#e53e3e" }}>
                ...
                <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
                    <div style={{ background: "white", borderRadius: "16px", padding: "28px", width: "440px", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
                        ...
                        {[
                            ...
                            { label: "Student ID", key: "studentId", type: "text", placeholder: "e.g. STU-2024-006" },
                        ].map(f => (
